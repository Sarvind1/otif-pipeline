...
class StageCalculator:
    ...
    def calculate_adjusted_timestamp(self, stage_id: str, po_row: pd.Series) -> Tuple[Optional[datetime], Dict[str, Any]]:
        ...
        print (has_projected_precedence)
    ...
    def _process_preceding_stages(
        ...
        for prec_stage_id in preceding_stage_ids:
            ...
            if prec_stage_id in self.config.stages:
                ...
                has_projected_precedence = self._process_regular_stage(prec_stage_id, po_row, calc_details, 
    ...
    def _process_date_expression(
        ...
        try:
            ...
            method = "Actual" if is_actual else "Adjusted"
    ...
    def _calculate_delay(self, stage_id: str, calc_details: Dict[str, Any]) -> None:
        ...
        # Calculate delay for Projected method with Actual/Adjusted precedence
        elif method == "Projected" and precedence_method == "Actual/Adjusted" and target_ts:
            try:
                target_dt = datetime.fromisoformat(target_ts)
                today_dt = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
                delay_days = (today_dt - target_dt).days
                calc_details["delay"] = delay_days
                print(f"Delay (vs today): {delay_days} days")
                calc_details["calculation_source"] += f" - current status: {'Current Status'}"

            except Exception as e:
                print(f"ERROR calculating delay vs today for {stage_id}: {e}")
